#!/bin/bash
# PreToolUse hook: blocks costly tool calls when GLM quota guard says PAUSE/STOP.
# Reads only the cached pause-state.json written by glm_quota_decide.py — no
# network calls here, so this stays fast enough for a per-tool-call hook.
# Fails open (allows the call) if state is missing, stale, or unreadable.
...
reason=$(jq -r '.reason // "GLM quota guard active"' "$STATE_FILE" 2>/dev/null)
...
message="GLM quota guard (${decision}): ${reason} Resume at: ${resume_local}"
